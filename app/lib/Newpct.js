...
const url = require('../config').newpct;
const vm = require('vm');
...
async function getTVShowEpisodeTorrentUrl(tvShowUrl, season, episode, page = 1) {
  ...
  return getTVShowEpisodeTorrentUrl(tvShowUrl, season, episode, nextPage);
}

module.exports = {
  async search(query, page) {
    const queryUrl = `${url}/index.php`;
    ...
    const total = parseInt($('.page-box h3 strong').text().match(/\((.*?)\)/gi)[0].replace(/\(/gi, '').replace(/\)/gi, '').trim(), 10);
    return {
      torrents,
      total
    };
